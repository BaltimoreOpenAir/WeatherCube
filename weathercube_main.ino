...
{ long toc = millis();
  ...
  if (eeprom_write_location < 1024 * 128 - 64) {
    device = EEP0;
  }
  // second case: eeprom write location needs to move to eeprom2
  else if (eeprom_write_location < 1024 * 128) {
    device = EEP1;
  ...
  for (int i = 0; i < DATA_ARRAY_SIZE * 2; i += 2) {
    ...
    writeEEPROMdouble(device, eeprom_write_location, (data_array[i / 2] + 32768)); // note: save as signed integer
  ...
  if (loop_counter > loop_minimum) { //loop_minimum) {
    ...
    for (int reading_counter = 0; reading_counter < loop_counter + 1; reading_counter++) {
      ...
      for (int i = 0; i < DATA_ARRAY_SIZE * 2; i += 2) {
        ...
        byte two_e = readEEPROM(device, eeprom_write_location + i);
        byte one_e = readEEPROM(device, eeprom_write_location + i + 1);
