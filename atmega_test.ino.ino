...
void loop() // run over and over
{
  ...
  for (int i=0; i< DATA_ARRAY_SIZE; i++){Serial.println(data_array[i]);}
  //long data = data_array; 
  //long data[6] = {145.2, 3.45, 5.2, 350.0, 405.0}; // t, h, battery, o, n, s, h 
  ////-----------save readings------------------
  for (int i = 0; i < DATA_ARRAY_SIZE*2; i+=2) {
    writeEEPROMdouble(EEP0, 64 + i + loop_counter*32, data_array[i]);
  }
...
if (loop_counter > 6*24){
  ...
  for (int i = 0; i < DATA_ARRAY_SIZE*2; i += 2) {
    one_day_array[i] = readEEPROMdouble(EEP0,  64 + i + loop_counter*32);
  }
  digitalWrite(WIFI_EN, LOW);
  delay(5000); 
